...
const OT_TOOLS = [
  ...
  { name: 'Email Validator',         path: 'email-validator',       cat: '🌐 Network & Web',        icon: '📧', desc: 'Check email syntax, MX records and disposable/role-based flags',       isNew: true },
  { name: 'Email Deliverability',    path: 'email-deliverability',  cat: '🌐 Network & Web',        icon: '✉️', desc: 'SPF, DKIM, DMARC, MX, MTA-STS and BIMI with a config score',           isNew: true },
  { name: 'DNSBL Blacklist Checker', path: 'dnsbl-checker',         cat: '🌐 Network & Web',        icon: '🚫', desc: 'Is an IP on Spamhaus, SpamCop, Barracuda or other blocklists?',        isNew: true },
  { name: 'Disposable Email Detector', path: 'disposable-email',    cat: '🌐 Network & Web',        icon: '🗑️', desc: 'Flag throwaway/burner email domains — single or bulk list',            isNew: true },
  { name: 'Temp Inbox',              path: 'temp-inbox',            cat: '🌐 Network & Web',        icon: '📨', desc: 'Generate a disposable email address and read incoming mail',           isNew: true },
...
// One footer for the hub, every tool page and pages/*. Injected here rather than
// pasted into 130+ files so the wording and links have a single source.
// The flag is the same inline SVG the hub footer always carried (yellow field, three red
// stripes) — an SVG, not an emoji, so it renders identically on every OS.
const OT_GRIN_PATH = 'M135.7,63.5c-2-4.6-3.9-13.6-9.1-15.6c-6.7-2.6-10.9,14.3-11.9,18.6h-1c-1.7-7.3-4-17.7-12-20c-3.7,15.6,3.9,32.5,11,46c7.6-3.6,11.3-15,12-23h1l8,24c6.4-1.8,8.7-8.2,11-14c5-12.4,9.2-27.7,6-41C141.1,41.2,138.2,55,135.7,63.5 M39.7,95.5c8.6-4.1,12.7-17.1,14-26h1c1,4.3,3.7,15.4,9.1,16.1c6.8,0.9,10.4-14.3,10.9-19.1h1c2.2,7.5,4.5,17.3,12,21c2.8-11.7-0.5-24.1-4.8-35c-1.1-2.8-3.3-10.4-7.2-10.4c-6.3,0-9.9,16.7-11,21.4h-1l-8-24C41.6,46.2,34,82,39.7,95.5 M24.7,107.5c10.4,45.6,58.6,71.4,102,52.1c15.8-7,28.3-19.5,35.5-35.1c1.9-4,5.5-11.4,3.4-16.7c-2.1-5.3-22.8,3.4-27.9,5.6c-0.7,0.3-1.2,1-1.4,1.8c-0.3,2.2,1.2,4.3,3.4,4.6c0.1,0,0.2,0,0.3,0l8.7,0.7c-15.7,30.4-58.1,43.8-88,19.5C54,134.6,48,128.2,44,120.5c-1.9-3.7-3.7-8.4-7-11C33.8,107.1,29,106.3,24.7,107.5L24.7,107.5z';
function renderSiteFooter() {
  if (document.querySelector('.site-footer')) return; // page brought its own
  const root = _otRootPath();
  const path = window.location.pathname;
  const onHub    = !/\/(tools|pages)\//.test(path);
  const onDonate = path.endsWith('/donate.html');
  const flag = `<svg class="site-footer-flag" viewBox="0 0 27 18" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Yellow flag with three red stripes"><rect width="27" height="18" fill="#FFCD00"/><rect y="4" width="27" height="2" fill="#DA251D"/><rect y="8" width="27" height="2" fill="#DA251D"/><rect y="12" width="27" height="2" fill="#DA251D"/></svg>`;
  const grin = `<svg class="site-footer-grin" viewBox="0 0 191 191" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="95.5" cy="95.5" r="95.5"/><circle fill="#FEF102" cx="95.3" cy="95.5" r="85.3"/><path d="${OT_GRIN_PATH}"/></svg>`;
  const links = [];
  if (!onHub)    links.push(`<a href="${root}">🛠️ All tools</a>`);
  if (!onDonate) links.push(`<a href="${root}pages/donate.html" class="site-footer-donate">Donate me 1 ツ ${grin}</a>`);
  links.push(`<a href="${root}pages/privacy.html">Privacy &amp; Cookie Notice</a>`);
  const footer = document.createElement('footer');
  footer.className = 'site-footer';
  footer.innerHTML =
    `<div class="site-footer-inner">` +
      `<p>From Saigon with ❤️ ${flag}</p>` +
      `<p class="site-footer-links">${links.join('')}</p>` +
    `</div>`;
